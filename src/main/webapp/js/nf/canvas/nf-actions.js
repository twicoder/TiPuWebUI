    ...
    var nfActions = {
        ...
        enable: function (selection) {
            ...
            if (componentsToEnable.empty()) {
                nfDialog.showOkDialog({
                    headerText: '使能组件',
                    dialogContent: '没有选择合适的组件，请选择要启用的组件，并确保它们没有在运行。'
                });
            } else {
                var enableRequests = [];
        ...
        disable: function (selection) {
            ...
            if (componentsToDisable.empty()) {
                nfDialog.showOkDialog({
                    headerText: '禁止组件',
                    dialogContent: '没有选择合适的组件，请选择要禁止的组件，并确保它们没有在运行。'
                });
            } else {
                var disableRequests = [];
        ...
        start: function (selection) {
            if (selection.empty()) {
                ...
                if (componentsToStart.empty()) {
                    nfDialog.showOkDialog({
                        headerText: '启动组件',
                        dialogContent: '没有选择合适的组件，请选择要启动的组件，并确保它们没有在运行。'
                    });
                } else {
                    var startRequests = [];
        ...
        stop: function (selection) {
            if (selection.empty()) {
                ...
                if (componentsToStop.empty()) {
                    nfDialog.showOkDialog({
                        headerText: '停止组件',
                        dialogContent: '没有选择合适的组件，请选择要停止的组件。'
                    });
                } else {
                    var stopRequests = [];
        ...
        'delete': function (selection) {
            if (nfCommon.isUndefined(selection) || selection.empty()) {
                nfDialog.showOkDialog({
                    headerText: '删除组件',
                    dialogContent: '没有选择合适的组件，请选择要删除的组件，并确保它们没有在运行。'
                });
            } else {
                if (selection.size() === 1) {
                    var selectionData = selection.datum();
        ...
        emptyQueue: function (selection) {
            ...
            nfDialog.showYesNoDialog({
                headerText: '清空队列',
                dialogContent: '确定要清空队列吗？所有当前等待的流文件将会被删除。',
                noText: 'Cancel',
                yesText: 'Empty',
                yesHandler: function () {
        ...
        alignVertical: function (selection) {
            ...
            if (nfCanvasUtils.canModify(selection) === false) {
                nfDialog.showOkDialog({
                    headerText: '组件位置',
                    dialogContent: '要修改选择的组件，首先需要被授权。'
                });
                return;
            }
                    ...
                    click: function () {
                        ...
                        nfSnippet.create(snippet).done(function (response) {
                            ...
                            $.ajax({
                                ...
                                nfDialog.showOkDialog({
                                    headerText: '创建模版',
                                    dialogContent: "模版 '" + nfCommon.escapeHtml(templateName) + "' 创建成功。"
                                });
                            }).always(function () {
